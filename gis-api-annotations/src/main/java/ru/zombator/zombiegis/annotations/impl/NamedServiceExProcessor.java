 ...
 * Portions Copyrighted 2014 Nikolai Amelichev
 */
package ru.zombator.zombiegis.annotations.impl;
...
public final class NamedServiceExProcessor extends AbstractServiceProviderProcessor {
    ...
    public Set<String> getSupportedAnnotationTypes() {
        ...
        all.add(NamedServiceExDefinition.class.getName());
    ...
    protected boolean handleProcess(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        for (Element e : roundEnv.getElementsAnnotatedWith(NamedServiceExDefinition.class)) {
            NamedServiceExDefinition nsd = e.getAnnotation(NamedServiceExDefinition.class);
            ...
            while (m.find()) {
                ...
                TypeMirror enumType = processingEnv.getElementUtils().getTypeElement("java.lang.Enum").asType(); // NOI18N
                ...
                if (processingEnv.getTypeUtils().isAssignable(toCheck, stringType) ||
                    processingEnv.getTypeUtils().isAssignable(toCheck, enumType)) {
                    continue;
                }
                ArrayType arrEnumType = processingEnv.getTypeUtils().getArrayType(enumType);
                ...
                if (processingEnv.getTypeUtils().isAssignable(toCheck, arrStringType) ||
                    processingEnv.getTypeUtils().isAssignable(toCheck, arrEnumType)) {
                    continue;
                }
                processingEnv.getMessager().printMessage(
                    Diagnostic.Kind.ERROR,
                        ...
                        "' does not return String, String[], Enum or Enum[]",
                        e
                );
            }
        ...
        for (String className : index) {
            ...
            for (Element e : roundEnv.getElementsAnnotatedWith(c)) {
                ...
                NamedServiceExDefinition nsd = c.getAnnotation(NamedServiceExDefinition.class);
                int cnt = 0;
            ...
            for (int i = 0; i < arr.size(); i++) {
                ...
                if (m.find()) {
                    ...
                    if (obj instanceof String) {
                        ...
                        arr.set(i, substitute(path, m, ((Enum<?>) obj).name()));
                    } else if (obj instanceof Enum[]) {
                        Enum<?>[] subs = (Enum[]) obj;
                        arr.set(i, substitute(path, m, subs[0].name()));
                        for (int j = 1; j < subs.length; j++) {
                            arr.add(substitute(path, m, subs[j].name()));
                        }
                    } else {
    ...
    private static void searchAnnotations(Set<String> found, boolean canonicalName) {
        try {
            Enumeration<URL> en = NamedServiceExDefinition.class.getClassLoader().getResources(PATH);
